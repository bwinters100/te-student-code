...
public class PracticePart1 {
	...
	*/
	public double Q01_removeFraction(double numerator, double denominator) {
		double removeFraction = numerator / denominator;
		int removeDouble = (int) removeFraction;
		return removeDouble;
	}
...
//		return false;
	...
	*/
	public Map<String, Integer> Q11_idealPets(String[] suggestedPets) {
		Map<String, Integer> idealPetCounts = new HashMap<>();

		for (String pet : suggestedPets) {

			Integer count = idealPetCounts.get(pet);

			if (count == null) {
				idealPetCounts.put(pet, 1);
			} else {
				count = count + 1;
				idealPetCounts.put(pet, count);
			}
		}
		return idealPetCounts;
	}
}
