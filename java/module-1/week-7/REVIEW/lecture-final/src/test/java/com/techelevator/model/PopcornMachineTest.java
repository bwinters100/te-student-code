...
public class PopcornMachineTest {
    ...
    public void getChange() {
        ...
        Assert.assertEquals(0.00, machine.getChange(), DELTA);
        // TODO : TDD : As a user, I expect the that if I add money then I can get the same amount back as change
        machine.addMoney(new BigDecimal("20.00"));
        Assert.assertEquals(20.00, machine.getChange(), DELTA);
    ...
    @Test
    public void buyPopcorn() throws InsufficientFundsException {
        //Arrange
        PopcornMachine machine = new PopcornMachine(10,7,5);
        ...
        orders.add(order);
        Popcorn[] expectedResult = {new Popcorn(Popcorn.LARGE_BUCKET, true, true)};
        ...
        Popcorn[] results = machine.buyPopcorn(orders);
        ...
        Assert.assertArrayEquals(expectedResult, results);
